...
states_tpi_num = 6
states_tpi = np.zeros((t_num), dtype=int)
for i in range(t_num):
    if states[i] == 0 and tpi_data[i] == 0:
        states_tpi[i] = 0
    elif states[i] == 0 and tpi_data[i] == 1:
        states_tpi[i] = 1
    elif states[i] == 1 and tpi_data[i] == 0:
        states_tpi[i] = 2    
    elif states[i] == 1 and tpi_data[i] == 1:
        states_tpi[i] = 3
    elif states[i] == 2 and tpi_data[i] == 0:
        states_tpi[i] = 4
    else:
        states_tpi[i] = 5
    # end if
# end for
print('\nNo. ENSO_Tpi states =', states_tpi_num)
...
# Fit model
m1 = smodel.Model_UMM()
m1.fit(states_tpi_num, states_tpi, num_symbols, symbols, adjust_zero_states=True)
m1.print()
...
# state0 = 0
# state0 = 1
state_tpi0 = states_tpi[-1]  # Last observed state


# Forecast from model by samples
states_tpi1, symbols1 = m1.sample(r1_num, t1_num, state0=state_tpi0, seed=1)
if make_outputs:
    ...
    states_tpi1p = np.zeros((t1_num,states_tpi_num), dtype=float)
    symbols1p = np.zeros((t1_num,num_symbols), dtype=float)
    for i in range(t1_num):
        for j in range(states_tpi_num):
            states_tpi1p[i,j] = np.count_nonzero(states_tpi1[i,:] == j) / r1_num
        # end for
    ...
    ax.set_ylabel('sp')
    ax.grid()
    ax.plot(t1, states_tpi1p, label=['st{}'.format(k) for k in range(states_tpi_num)])
...
state_tpip1, symbolp1 = m1.probs(t1_num, state0=state_tpi0)
if make_outputs:
    ...
    ax.grid()
    ax.plot(t1, state_tpip1, label=['st{}'.format(k) for k in range(states_tpi_num)])
    fig.legend(loc='upper center', frameon=False,  ncol=10, bbox_to_anchor=(0.5, 0.95))
    ax.set_xlim([0.0, 10.0])
    ax.set_ylim([0.0, 1.0])
    plt.show()
...
state_BSS, symbol_BSS = m1.BSS(t1_num, states_tpi, symbols)
if make_outputs:
