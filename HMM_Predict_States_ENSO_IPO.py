#==============================================================================
#
# Hidden Markov Model - Generate from known states - ENSO+IPO
#
#------------------------------------------------------------------------------
#
# Notes:
...
# Output plot files
plot_gen1_states = 'plots/{CATCHMENT}.gen1_states.png'
plot_gen1_symbols = 'plots/{CATCHMENT}.gen1_symbols.png'
# ----------
plot_rel_diag = 'plots/{CATCHMENT}.rel_diag.png'
plot_roc_curve = 'plots/{CATCHMENT}.roc_curve.png'
plot_rel_diag_all = 'plots/{CATCHMENT}.rel_diag_all.png'
plot_roc_curve_all = 'plots/{CATCHMENT}.roc_curve_all.png'
# ----------
plot_fcst_states_p = 'plots/{CATCHMENT}.fcst_states_p.png' 
plot_fcst_symbols_p = 'plots/{CATCHMENT}.fcst_symbols_p.png'
plot_fcst_states_r = 'plots/{CATCHMENT}.fcst_states_r.png'
plot_fcst_symbols_r = 'plots/{CATCHMENT}.fcst_symbols_r.png'
# ----------
...
c_data_file = 'QLD_prediction_May_Apr.csv'
...
for i in range(t_data_num):
    if data_df['QLD_Apr_Mar'][i] < data_df['QLD_Apr_Mar'].quantile(0.5):
        o_data[i] = 0
    else:
        o_data[i] = 1
    # end if
...
for i in range(t_data_num):
    if data_df['TPI_Jul_Jun_+01'][i] >-0.01:
        tpi_data[i] = 0
    #elif data_df['TPI_May_02_Apr_01'][i] <= 644.8:
    #    tpi_data[i] = 1
    else:
        tpi_data[i] = 1
    # end if
...
for i in range(t_data_num):
    if s_data[i] == 0 and tpi_data[i] == 0:
        s_tpi_data[i] = 0
    elif s_data[i] == 0 and tpi_data[i] == 1:
        s_tpi_data[i] = 1
    elif s_data[i] == 1 and tpi_data[i] == 0:
        s_tpi_data[i] = 2    
    # elif s_data[i] == 1 and tpi_data[i] == 1:
    #     s_tpi_data[i] = 3
    # elif s_data[i] == 2 and tpi_data[i] == 0:
    #     s_tpi_data[i] = 4
    else:
        s_tpi_data[i] = 3
    # end if
...
t_matrix = GetTransitionMatrix(s_tpi_data_num, s_tpi_data, adjust_zero_states=False)
print('\nTransmission matrix:')
print(t_matrix)
...
# Set up model
model = hmm.CategoricalHMM(n_components=s_tpi_data_num)  # 3 hidden states
model.n_features = o_data_num # 3 observed symbols
...
prob_true_ENP, prob_pred_ENP = calibration_curve(ground_truth_ENP, predictions_ENP, n_bins=10, strategy='uniform')
prob_true_ENN, prob_pred_ENN = calibration_curve(ground_truth_ENN, predictions_ENN, n_bins=10, strategy='uniform')
prob_true_LNP, prob_pred_LNP = calibration_curve(ground_truth_LNP, predictions_LNP, n_bins=10, strategy='uniform')
prob_true_LNN, prob_pred_LNN = calibration_curve(ground_truth_LNN, predictions_LNN, n_bins=10, strategy='uniform')


# Plot reliability diagram
plt.figure()
plt.plot(prob_pred_ENP, prob_true_ENP, marker='o', color='red', label='ENP curve')
plt.plot(prob_pred_ENN, prob_true_ENN, marker='o', color='brown', label='ENN curve')
plt.plot(prob_pred_LNP, prob_true_LNP, marker='o', color='green', label='LNP curve')
plt.plot(prob_pred_LNN, prob_true_LNN, marker='o', color='blue', label='LNN curve')
plt.plot([0, 1], [0, 1], color='grey', linestyle='--')
plt.plot([0, 1], [0.125, 0.625], color='grey', linestyle='dashdot', label='No skill')
plt.axhline(y=0.25,  color='grey', linestyle='dotted', label='No Resolution')
plt.axvline(x=0.25,  color='grey', linestyle='dotted', label=None)
x1=[0.25,1,1,0.25]
y1=[0.25,0.625,1,1]
x2=[0,0.25,0.25,0]
y2=[0,0,0.25,0.125]
...
plt.title('QLD Reliability Diagram for 4-states',fontweight='bold')
plt.legend(loc='lower right')
plt.show()


# Calculate ROC curve
fpr_ENP, tpr_ENP, thresholds = roc_curve(ground_truth_ENP, predictions_ENP)
fpr_ENN, tpr_ENN, thresholds = roc_curve(ground_truth_ENN, predictions_ENN)
fpr_LNP, tpr_LNP, thresholds = roc_curve(ground_truth_LNP, predictions_LNP)
fpr_LNN, tpr_LNN, thresholds = roc_curve(ground_truth_LNN, predictions_LNN)


# Calculate AUC (Area Under Curve)
roc_auc_ENP = auc(fpr_ENP, tpr_ENP)
roc_auc_ENN = auc(fpr_ENN, tpr_ENN)
roc_auc_LNP = auc(fpr_LNP, tpr_LNP)
roc_auc_LNN = auc(fpr_LNN, tpr_LNN)


# Plot ROC curve
plt.figure()
plt.plot(fpr_ENP, tpr_ENP, color='red', lw=2, label=f'ENP_ROC curve (AUC = {roc_auc_ENP:.2f})')
plt.plot(fpr_ENN, tpr_ENN, color='brown', lw=2, label=f'ENN_ROC curve (AUC = {roc_auc_ENN:.2f})')
plt.plot(fpr_LNP, tpr_LNP, color='green', lw=2, label=f'LNP_ROC curve (AUC = {roc_auc_LNP:.2f})')
plt.plot(fpr_LNN, tpr_LNN, color='blue', lw=2, label=f'LNN_ROC curve (AUC = {roc_auc_LNN:.2f})')
plt.plot([0, 1], [0, 1], color='grey', lw=2, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate',fontweight='bold')
plt.ylabel('True Positive Rate',fontweight='bold')
plt.title('QLD Relative Operating Characteristic for 4-states (ROC)',fontweight='bold')
plt.legend(loc='lower right')
plt.show()
...
S1 = np.zeros([11,s_tpi_data_num], dtype=float)
S1[0,:] = np.array([ 1,0,0, 0 ])
...
sp0 = np.array([1.0, 0.0, 0.0, 0.0], dtype=float) #2021/22 observed state


# Forecast starting state (first state is numbered 0)
s0 = s_tpi_data[133] #2021/22 observed state
...
op0 = np.array([ 0,   1  ], dtype=float) #2021/22 observed sysmbol
op1[0,:] = op0
for l in range(1, t1_num):
